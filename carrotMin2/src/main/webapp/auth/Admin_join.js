...
$(function() {
	...
	$('#btnSelect').click(function() {
		if ($('#inputId').val().length == 0) {
			...
			alert('비밀번호 입력은 필수 입력 항목입니다.');
			return false;
		} else if ($('#inputPhoneNum').val().length == 0) {
		...
		$.post('../json/auth3/signup.do'  
		, {  
			//서버에 보낼 데이터를 객체에 담아 넘긴다 
			aid : $('#inputId').val(),
			apwd : $('#inputPassword3').val(),
			atel : $('#inputPhoneNum').val(),
			aname : $('#inputRepresentative').val(),
		...
		if (resultMap.status == "success") {
				...
				location.href = '../auth/Admin_login.html';
			} else {
 ...
 $(function() {
	$("#inputId").keyup(function() { //아이디 유효성검사
		if(checkId()){
			$.get("http://192.168.123.107:3000/Adminidcheck", {
				id : $('#inputId').val()
			}, function(data) {
